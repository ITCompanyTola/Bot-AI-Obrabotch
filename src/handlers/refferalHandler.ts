...
export function registerRefferal(bot: Telegraf<BotContext>) {
  bot.action("create_refferal", async (ctx) => {
    ...
    if (isRefferalCreated) {
      const refferalLink =
        `https://t.me/Obrabotych_bot?start=${await Database.getRefferalLink(
          userId
        )}`.trim();
              ...
              [
                {
                  ...
                  switch_inline_query: `Крутой ИИ-бот для разных генераций 👇\n${refferalLink}`,
                },
              ],
              [{ text: "Главное меню", callback_data: "main_menu" }],
            ],
