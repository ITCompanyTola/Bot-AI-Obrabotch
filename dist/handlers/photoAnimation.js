...
exports.registerPhotoAnimationHandlers = registerPhotoAnimationHandlers;
...
📸 Вам остаётся только отправить фотографии — всё остальное мы берём на себя.
  `.trim();
        await ctx.telegram.sendMessage(userId, orderVideoMessage, telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.url('Заказать видео подарок', 'https://t.me/khodunow')],
    ...
    bot.action('order_video_gift', async (ctx) => {
        ...
        if (!userId)
            return;
        await ctx.telegram.sendMessage(userId, '💬 Свяжитесь с нами для заказа видео подарка:', telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.url('Написать', 'https://t.me/khodunow')],
            [telegraf_1.Markup.button.callback('Главное меню', 'main_menu')]
        ]));
    });
...
📹 Требуется: ${constants_1.PRICES.PHOTO_ANIMATION} ₽</blockquote>`, {
                parse_mode: 'HTML',
        ...
        (0, klingService_1.processVideoGeneration)(ctx, userId, photoFileId, prompt);
        userStates.delete(userId);
    });
}
