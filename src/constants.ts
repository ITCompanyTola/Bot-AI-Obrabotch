...
export const mainMenuKeyboard = [
  ...
  [Markup.button.callback("💃 Создать танец", "trend_video")],
...
- создать <b><i>трендовые видео с танцами</i></b> 💃
...
export const TELEGRAM_CHANNEL_MESSAGE =
  "\n\n<b>Следите за обновлениями в нашем Telegram-канале:</b>\nhttps://t.me/+4gfCmvy5mS82NjAy";

export const TELEGRAM_CHANNEL_ID = "@ai_lumin";
...
Ключевые правила:
- Полностью сохраняй смысл и все желаемые действия/эффекты из оригинала. Не добавляй новых деталей, которых не было в описании пользователя.
- Формулируй действия и эффекты конкретнее и яснее, устраняя неоднозначности, но только на основе того, что уже сказано.
- Если пользователь описывает движения или эмоции людей, но не указал каких именно — применяй действие ко всем людям, видимым на фото.
- Описывай действия и эффекты так, чтобы они применялись строго к элементам, уже присутствующим на фото (люди, объекты, фон).
- Если описываются эффекты объектов (гирлянда, огонь, вода и т.д.) — явно указывай, что это анимация существующего элемента.
- Естественно и органично вплетай в описание следующие требования (не списками и не в конце):
  • движения плавные, спокойные, реалистичные, естественные;
  • никаких искажений лиц, артефактов, резких или преувеличенных эмоций;
  • лица, мимика и черты (глаза, губы, нос, зубы и т.д.) взяты строго с фотографии один в один, без малейших изменений;
  • внешность людей, одежда, окружение, освещение, ракурс и стиль сцены остаются абсолютно неизменными;
  • ничего не добавляется и не удаляется — ни люди, ни объекты.

Выводи ТОЛЬКО финальный промпт. Без кавычек, пояснений, нумерации и лишних слов.
...
export const getTrendVideoMessage = (balance: number) =>
  `
💃 <b>Наш бот умеет создавать трендовые танцевальные видео!</b>

Вы можете создать вирусный клип в стиле популярных танцев — с людьми, детьми или питомцами💃

1️⃣ Нажмите кнопку
<b>«💃 Создать видео»</b>
2️⃣ <b>Отправьте одну картинку* в бот</b>
3️⃣ <b>Подождите</b> — в течении 15 минут бот пришлёт готовое видео 🎬

<blockquote>💰 Ваш баланс: ${balance.toFixed(2)} ₽
💃 Создание 1 видео: ${PRICES.TREND_VIDEO}₽</blockquote>

❗️<b>* - бот принимает только одну картинку за раз</b>☝🏻

❗️<b>- из-за особенностей нейросети лицо и окружение может искажаться</b>`.trim();

export const TREND_VIDEO_START = `
<b>Пример ⤴️</b>

Отправьте <b><i>картинку</i></b> — и бот заставит персонажа танцевать в стиле популярных трендов 💃`.trim();

export const TREND_VIDEO_START_ONLY_TEXT =
  `Отправьте <b><i>картинку</i></b> — и бот заставит персонажа танцевать в стиле популярных трендов 💃`.trim();
