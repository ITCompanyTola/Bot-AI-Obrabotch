...
exports.registerMusicCreationHandlers = registerMusicCreationHandlers;
exports.showMusicAdvancedParams = showMusicAdvancedParams;
const telegraf_1 = require("telegraf");
const database_1 = require("../database");
const constants_1 = require("../constants");
const sunoService_1 = require("../services/sunoService");
async function showMusicAdvancedParams(ctx) {
    ...
    await ctx.editMessageText(advancedParamsMessage, telegraf_1.Markup.inlineKeyboard([
        [telegraf_1.Markup.button.callback('Пропустить', 'music_skip_params')],
        [telegraf_1.Markup.button.callback('Назад', 'music_back_to_style')]
    ]));
}
function registerMusicCreationHandlers(bot, userStates) {
...
🎵 Создать 1 трек = ${constants_1.PRICES.MUSIC_CREATION}₽</blockquote>
    `.trim();
        await ctx.editMessageText(musicCreationMessage, {
            parse_mode: 'HTML',
            ...telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('🎶 Начать творить', 'start_music_creation')],
                [telegraf_1.Markup.button.callback('Видео-инструкция', 'music_video_instruction')],
    ...
    bot.action('start_music_creation', async (ctx) => {
        ...
        await ctx.editMessageText('— Отправьте ниже 1-2 предложения о том, о чем хотите создать музыку или напишите полный текст для музыки', telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('Назад', 'music_creation')]
        ]));
    });
    ...
    bot.action('music_style_rock', async (ctx) => {
        ...
        const balance = await database_1.Database.getUserBalance(userId);
        const hasBalance = await database_1.Database.hasEnoughBalance(userId, constants_1.PRICES.MUSIC_CREATION);
        if (!hasBalance) {
            const paymentMessage = `
💰 Ваш баланс: ${balance.toFixed(2)} ₽
🎵 Создание 1 трека = ${constants_1.PRICES.MUSIC_CREATION}₽
...
Выберете способ оплаты ⤵️
    `.trim();
        ...
        const deducted = await database_1.Database.deductBalance(userId, constants_1.PRICES.MUSIC_CREATION, `Создание музыки: ${userState.musicText.substring(0, 50)}...`);
        if (!deducted) {
            await ctx.reply('❌ Ошибка списания средств. Попробуйте позже.');
            userStates.delete(userId);
            return;
        }
        (0, sunoService_1.processMusicGeneration)(ctx, userId, userState.musicText, userState.musicStyle);
        userStates.delete(userId);
    });
...
Выберете способ оплаты ⤵️
    ...
    bot.action('music_back_to_style', async (ctx) => {
        ...
        await ctx.editMessageText(styleMessage, telegraf_1.Markup.inlineKeyboard([
            [
