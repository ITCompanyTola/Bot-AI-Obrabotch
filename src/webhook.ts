...
app.post("/webhook/yookassa", async (req, res) => {
  try {
    ...
    if (
      ...
      console.log(
        `💳 Успешная оплата: ${paymentId}, сумма: ${amount}₽, пользователь: ${userId}`
      );
      ...
      try {
        const refferalData: UserRefferalData =
          await Database.getUserRefferalData(userId);
        ...
        console.log(
          `🔑 Реферальные данные пользователя: userRefferalKey=${userRefferalKey}, refferalKeyUsed=${refferalKeyUsed}`
        );
        if (userRefferalKey != undefined && refferalKeyUsed != undefined) {
          if (!refferalKeyUsed) {
            const reffererUserId = await Database.getUserIdByRefferalKey(
              userRefferalKey
            );
...
app.post("/webhook/robokassa", async (req, res) => {
  try {
    const { OutSum, InvId, SignatureValue, Shp_user_id } = req.body;

    console.log("📩 Robokassa webhook:", req.body);

    // 1. Проверка подписи (PASS2)
    const crcString = `${OutSum}:${InvId}:${process.env.ROBOKASSA_PASS_2}:Shp_user_id=${Shp_user_id}`;
    ...
    try {
      const refferalData: UserRefferalData = await Database.getUserRefferalData(
        userId
      );
      console.log(refferalData);
      const userRefferalKey = refferalData?.source_key;
...
app.post("/robokassa/getUrl", async (req, res) => {
  try {
    const { userId, price } = req.body;
    const invoiceId = Date.now();
    const amount = price.toString() + ".00";
    ...
    await Database.savePendingPayment(userId, String(invoiceId), Number(price));

    res.status(200).send(paymentUrl);
  } catch (error) {
    console.error("❌ Ошибка обработки запроса на получение URl:", error);
    res.status(500).send("Internal Server Error");
  }
});
