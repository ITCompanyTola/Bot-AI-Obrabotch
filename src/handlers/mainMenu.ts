...
🏞 Количество генераций открыток из фото: <b>${
...
🏞 Количество генераций реставрации: <b>${
...
🏞 Количество генераций открыток из фото: <b>${
  ...
  bot.command("refferal_stats", async (ctx) => {
    try {
      const userId = ctx.from?.id;
      if (!userId) return;

      if (!(await Database.isAdmin(userId))) {
        await ctx.reply("❌ У вас нет прав для выполнения этой команды");
        return;
      }

      const stats = await Database.getUsersRefferalStats();

      const message = `
🔗 <b>Статистика рефералов</b>

👥 Кол-во реферальных ссылок: <b>${stats.totalCreatedRefferal}</b>
👥 Кол-во использованых ссылок: <b>${stats.totalUsedRefferal}</b>

💰 Сумма, полученная пользователями: <b>${(
        stats.totalUsedRefferal * 100
      ).toFixed(2)}₽</b>
      `.trim();

      await ctx.reply(message, { parse_mode: "HTML" });
    } catch (error) {
      console.log("Ошибка получения статистики рефералов:", error);
