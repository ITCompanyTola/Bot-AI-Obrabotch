...
<blockquote>💰 Ваш баланс: ${balance.toFixed(2)}₽</blockquote>
    ...
    await ctx.editMessageText("Выберите тип открытки", {
      reply_markup: {
        inline_keyboard: [
          ...
          [{ text: "Рождественские", callback_data: "my_postcards_christmas" }],
          [{ text: "Новогодние", callback_data: "my_postcards_photo" }],
  ...
  bot.action("my_postcards_christmas", async (ctx) => {
    try {
      await ctx.answerCbQuery();
    } catch (error: any) {
      if (!error.description?.includes("query is too old")) {
    ...
    const my_postcards_christmas = await Database.getUserChristmasPostcards(
      userId
    );

    if (my_postcards_christmas.length === 0) {
      await ctx.editMessageText(
        "📄 У вас пока нет рождественских открыток",
        Markup.inlineKeyboard([
    ...
    for (const postcard of my_postcards_christmas) {
      try {
        await ctx.telegram.sendPhoto(userId, postcard.file_id);
      } catch (error) {
        console.error("Ошибка отправки рождественской открытки:", error);
        await ctx.telegram.sendMessage(
          userId,
          `❌ Открытка недоступна (ID: ${postcard.id})`
        );
      }
    }
    ...
    await ctx.telegram.sendMessage(
      userId,
      `Ваши Рождественские Открытки (${my_postcards_christmas.length}):`,
      Markup.inlineKeyboard([[Markup.button.callback("Назад", "my_postcards")]])
    );
  });
  ...
  bot.action("my_postcards_text", async (ctx) => {
    ...
    for (const postcard of postcards_text) {
      try {
        await ctx.telegram.sendPhoto(userId, postcard.file_id, {
          caption: postcard.prompt
            ? `Описание: <blockquote><code>${postcard.prompt}</code></blockquote>`
            : undefined,
          parse_mode: "HTML",
        });
      } catch (error) {
      ...
      try {
        await ctx.telegram.sendVideo(userId, photo.file_id, {
          caption: photo.prompt
            ? `Описание:\n<blockquote><code>${photo.prompt}</code></blockquote>`
            : undefined,
          parse_mode: "HTML",
        });
      } catch (error) {
    ...
    for (const track of tracks) {
      try {
        await ctx.telegram.sendAudio(userId, track.file_id, {
          caption: track.prompt
            ? `Описание:\n<blockquote><code>${track.prompt}</code></blockquote>`
            : undefined,
          parse_mode: "HTML",
        });
      } catch (error) {
  ...
  bot.action("my_restorations", async (ctx) => {
    ...
    for (const restoration of restorations) {
      try {
        ...
        await ctx.telegram.sendMessage(
          userId,
          `❌ Реставрация недоступна (ID: ${restoration.id})`
        );
      }
    }
  ...
  bot.action("my_dm_photos", async (ctx) => {
    ...
    for (const dm_photo of all_dm_photos) {
      try {
        ...
        await ctx.telegram.sendMessage(
          userId,
          `❌ Фото Деда Мороза недоступно (ID: ${dm_photo.id})`
        );
      }
    }
    ...
    for (const dm_video of all_dm_videos) {
      try {
        await ctx.telegram.sendVideo(userId, dm_video.file_id, {
          caption: dm_video.prompt
            ? `Описание:\n<blockquote><code>${dm_video.prompt}</code></blockquote>`
            : undefined,
          parse_mode: "HTML",
        });
      } catch (error) {
        ...
        await ctx.telegram.sendMessage(
          userId,
          `❌ Видео Деда Мороза недоступно (ID: ${dm_video.id})`
        );
      }
    }
  ...
  bot.action("my_colorize", async (ctx) => {
    ...
    for (const colorized of all_colorized) {
      try {
        ...
        await ctx.telegram.sendMessage(
          userId,
          `❌ Фото с добавлением цвета недоступно (ID: ${colorized.id})`
        );
      }
    }
...
📌 <a href="https://docs.google.com/document/d/1lBw4BXuPKiFjXrRxeXnFBhJm_TTbsWd8iXoPO7Fw5YQ/edit?usp=sharing">Договор Оферты</a>
    ...
    await ctx.editMessageText(documentsMessage, {
      parse_mode: "HTML",
      ...Markup.inlineKeyboard([[Markup.button.callback("Назад", "profile")]]),
    });
  });
