...
- <b><i>Допустимо</i></b> присылать фото в купальнике или белье с нейтральным описанием вроде "Позирует на камеру"`.trim();
    ...
    if (userState?.step === "waiting_for_restoration_photo") {
      const photo = ctx.message.photo[ctx.message.photo.length - 1];
      const prompt =
        "Restore this old photo: improve sharpness, remove defects, but preserve the original colors without recoloring.";
    ...
    if (userState?.step === "waiting_for_colorize_photo") {
      const photo = ctx.message.photo[ctx.message.photo.length - 1];
      const prompt =
        "Convert a black-and-white photo to color and improve the quality and clarity of the photo";
  ...
  bot.on("text", async (ctx) => {
    ...
    if (userState?.step === "waiting_music_text") {
      ...
      console.log(
        `🎵 Сохранен текст музыки для пользователя ${userId}: "${musicText}"`
      );
    ...
    await ctx.reply(
      "Подождите немного — мы <b><i>улучшаем ваше описание</i></b>, чтобы результат получился максимально <b><i>качественным</i></b>🔥",
      {
        parse_mode: "HTML",
      }
    );
    ...
    try {
      ...
      await ctx.reply(
        "❌ Произошла ошибка в генерации описания. Попробуйте снова позднее."
      );
      return;
    }
    ...
    await ctx.reply(message, {
      ...
      reply_markup: {
        inline_keyboard: [
          ...
          [
            {
              text: `Улучшить еще раз ${4 - userState.regenPromptAttempts}/3`,
              callback_data: "regenerate_prompt",
            },
          ],
  ...
  bot.action("regenerate_prompt", async (ctx) => {
    ...
    let updatedPromptMessage;
    try {
      updatedPromptMessage = await updatePrompt(
        userState.prompt,
        photoUrlString
      );
    } catch (error) {
  ...
  bot.action("confirm_prompt", async (ctx) => {
    ...
    console.log(
      `📝 Сохранен промпт для пользователя ${userId}: "${userState.generatedPrompt}"`
    );
...
Выберите способ оплаты ⤵️`.trim();
    ...
    if (
      ...
      userState.generatedPrompt == undefined
    )
      return;
    processVideoGeneration(
      ctx,
